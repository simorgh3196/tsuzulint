...
| :--- | ---: | ---: | ---: | ---: | ---: |
| small      |   144.5 ms ±   1.3 |    183.5 ms ±  32.2 |     395.1 ms ± 28.0 |   1.27x | 0.46x |
| medium     |   170.5 ms ±  30.2 |    240.9 ms ±   8.9 |     516.4 ms ± 39.3 |   1.41x | 0.47x |
| large      |   227.9 ms ± 126.0 |   1213.4 ms ±  76.3 |    1301.9 ms ± 34.7 |   5.32x | 0.93x |
| monolithic |   527.0 ms (n=1)   | 106411.9 ms (n=1)   |   29560.2 ms (n=1)  | 201.91x | 3.60x |

(monolithic は WASM が遅すぎて hyperfine の 5 ランは現実的でないため `RUNS=1` で計測。
large の native は σ が大きく、安定計測には再ランを推奨。)

#### 観察

- **small/medium では WASM 経由でも textlint より圧倒的に速い**: corpus が小さい
  うちはホスト側の起動コストが支配的で、 WASM の MessagePack serialize
  オーバーヘッドは無視できる。
- **large 以上で WASM が急速に遅くなる**: 1 ブロックごとに `mi_heap_malloc` ↔
  Wasmtime memory のコピーと serialize/deserialize が走るため、ブロック数が増える
  と二次的に効いてくる。
- **monolithic は WASM rule pipeline の弱点が露呈**: 2MB の単一ファイルでは
  Native 比 200x、 textlint 比 3.6x も遅い。AST 全体を 1 リクエストにシリアライズ
  するコストがリニアでなく効くため。本番運用としては「巨大ファイルは複数に分割」
  というプラクティスが現実的。
- **WASM dispatch は機能としては正しく動く**: 旧 docs にあった「`is_node_type("Str")`
  で Paragraph を受け取って早期 return」の dispatch バグは解消済み (`crates/
  tsuzulint_ast::collect_nodes_by_type` を `crates/tsuzulint_core/src/file_linter.rs`
  が呼び、manifest 宣言通りのノード型を rule に渡す)。Native と同じ件数の
  diagnostic を返す。
