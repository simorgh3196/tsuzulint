...
pub fn lint_file_internal(
    ...
    // This is optimized by using binary_search on the sorted `global_rule_names` slice.
...
fn filter_overridden_diagnostics(
    ...
    if !global_rule_names.is_empty() {
        // Enforce the precondition that global_rule_names is sorted (O(M log M) handled earlier)
        debug_assert!(global_rule_names.windows(2).all(|w| w[0] <= w[1]));

        // Use binary_search on the sorted slice instead of allocating a HashSet per file.
        // This is significantly faster for diagnostic filtering in the hot path.
