...
pub fn lint_file_internal(
    ...
    // Since global_rule_names is sorted, we can use binary_search to avoid HashSet allocation.
...
fn filter_overridden_diagnostics(
    ...
    if !global_rule_ids.is_empty() {
        local_diagnostics.retain(|d| global_rule_ids.binary_search(&d.rule_id.as_str()).is_err());
    }
}
...
mod tests {
    ...
    fn test_filter_overridden_diagnostics() {
        ...
        let global_rule_ids = vec![global_id];
    ...
    fn test_filter_overridden_diagnostics_no_match() {
        ...
        let global_rule_ids = vec!["other-global-rule"];
