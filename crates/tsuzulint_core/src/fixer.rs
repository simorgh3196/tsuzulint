...
pub fn apply_fixes_to_file(
    ...
    let file = crate::file_linter::open_nonblocking(path)
    ...
    let content = bounded_read_to_string(&mut file_ref, crate::file_linter::MAX_FILE_SIZE as usize)
        .map_err(|e| {
...
mod tests {
    ...
    fn apply_fixes_to_file_too_large_actual_data() {
        ...
        // A FIFO is not a regular file and will be rejected by metadata.is_file().
