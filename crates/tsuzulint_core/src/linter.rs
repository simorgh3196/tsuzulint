...
mod tests {
    ...
    fn test_lint_file_multiple_global_rules() {
        ...
        assert!(result.timings.contains_key("test-rule"), "Missing timing for test-rule");
        assert!(result.timings.contains_key("test-rule-2"), "Missing timing for test-rule-2");
    }
}
