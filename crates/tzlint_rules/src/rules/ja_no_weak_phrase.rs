...
mod tests {
    ...
    fn flags_omoimasu_not_double_reported() {
        // 「思います」contains 「思」but must be reported once as the longer phrase,
        // not twice (once for 思います and once for the 思う substring if any).
        let diags = diagnose(&JaNoWeakPhrase::new(), "良いと思います。\n");
        // Expect exactly one diagnostic (for 思います); 思う does NOT appear as a standalone.
        assert_eq!(diags.len(), 1, "{diags:?}");
        assert!(
