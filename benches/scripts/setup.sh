...
if command -v npm >/dev/null 2>&1; then
  if [[ ! -d "${BENCHES_DIR}/configs/textlint/node_modules" ]]; then
    echo "==> Installing textlint dependencies (first run)"
    (cd "${BENCHES_DIR}/configs/textlint" && npm install --no-audit --no-fund --loglevel=error)
  else
    echo "==> textlint already installed (skip)"
  fi
else
